...
-- Remote-drift repair: the init schema attaches ad_slots_touch (before
-- update → touch_updated_at() which stamps NEW.updated_at), but the remote
-- ad_slots table (migrated from an older init) has no updated_at column —
-- so any UPDATE/ON CONFLICT DO UPDATE below died with 'record "new" has no
-- field "updated_at"'. Same idempotent-add pattern as the backup columns.
