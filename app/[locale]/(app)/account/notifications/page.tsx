...
import { getMyNotifications, getMyPrefs } from "@/lib/notify/queries";
...
export default async function Page() {
    ...
    const [items, prefs] = await Promise.all([getMyNotifications(), getMyPrefs()]);

    return (
        <div className="mx-auto max-w-6xl space-y-6 px-4 py-8 md:px-6 lg:py-12">
            <PageHeader title={t.title} description={t.description} />
            <NotificationsClient initial={items} prefs={prefs} dict={dict} locale={locale} />
        </div>
    );
}
