...
export function enqueueStaffAlert(
  event: Extract<NotifyPayload['event'], 'submission.received' | 'advertise.inquiry' | 'legal.takedown' | 'legal.contact' | 'legal.data_request' | 'content.correction' | 'content.updated' | 'poll.closed' | 'plan.failed' | 'translation.gap' | 'content.milestone' | 'digest.ready_for_review' | 'credential.hygiene' | 'storage.hygiene' | 'security.critical' | 'system.degraded'>,
